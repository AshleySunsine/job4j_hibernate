...
public class HbmCandidate {
    ...
    public static Candidateee getCandidate(int id, Session session) {
          Candidateee cand = (Candidateee) session.createQuery("from Candidateee c where c.id = :cid").setParameter("cid", 1).uniqueResult();
         System.out.println(cand);
         return cand;
    }

    public static Candidateee getCandidate(String name, Session session) {
        Candidateee cand = (Candidateee) session.createQuery("from Candidateee c where c.name = :cname").setParameter("cname", "Nikita").uniqueResult();
        System.out.println(cand);
        return cand;
    }
